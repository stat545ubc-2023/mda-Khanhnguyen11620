...
Similar to the apt_buildings database, three functions were applied to take a first look at the data: 
...
- Number of rows: 218
- Number of columns: 7
- There are 7 variables to give information about the water level of each station along with specific date, month and year. 
...
<!-------------------------- Start your work below ---------------------------->
I chose cancer_samples database for some reasons. 
- Firstly, the dataset includes 567 rows and 32 variables. The variables are mostly numerical variables which is often easier to work with in terms of basic statistical analysis, such as calculating means, medians, standard deviations, and correlations. Visualization of numerical data is typically straightforward, with options like histograms, scatter plots, and box plots that provide insights into data distribution and relationships. 
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
I want to explore the relationship between the measurements and the type of diagnosis. For example, is the radius (radius_mean) of malignant cancer sample likely to be larger than the one of benigh cancer sample? 
<!----------------------------------------------------------------------------->
...
In this project, we will use mostly the dplyr and gglot2 packages included in the tidyverse packages that we loaded above. dplyr is used for data wrangling while gglot2 is used for data visualization. 
...
2. Diagnosis: The target variable, indicating whether the biopsy is "M" (Malignant) or "B" (Benign).
...
The description of each variable is found on the UCI Machine Learning Repository website. 
...
ggplot(cancer_sample,aes(x=radius_mean, y=area_mean))+ 
  ...
  labs(title="Relationship between mean radius and mean area of samples ") + # aaded main title
  theme_minimal()
```
...
```{r}
ggplot(cancer_sample, aes(x= diagnosis, y = compactness_mean,colour = diagnosis)) +
  geom_boxplot()+
