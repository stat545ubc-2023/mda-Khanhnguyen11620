...
2. *What are the difference between the appearance of malignant and benign cancer samples (smoothness, compactness)/ Is*
...
In this project, we will use  the dplyr, gglot2 and tidyr packages included in the tidyverse packages. dplyr is used for data wrangling, gglot2 is used for data visualization and tidyr is used for data cleaning.
...
These data can directly give us an overview when comparing radius of each type of diagonsis (B =benign and M= malignant). 
By looking at the values of min value, max value and mean value of radius, we can see that the radius of benign cancer samples are generally smaller than the malignant ones. 
...
ggplot(cancer_sample,aes(x=radius_mean)) + # created the dataset and variable
  ...
  theme_minimal() # for minimalistic and clean design
```
The binwidth = 0.5 is the best when it offers a detailed information of the radius value's range. Binwidth = 2.5 is still good to look at but some information is lost ()

Binwidth = 5 is too large and hard to see
...
ggplot(cancer_sample,aes(x=smoothness_mean, y=compactness_mean))+ # set variable for each axis
  ...
  theme_minimal() 
```
This exercise is to answer the relationship between smoothness_mean and compactness_mean Based on this graph we can see a proportional relationship between smoothness and compactness. Besides, we also realize that: 
...
```
If the transparency (alpha = 1), the graph is difficult to see. Change to transparency to 0.5 or 0.6 will make the graph look nicer (see below)
...
Once again, These data can directly give us an overview when comparing number of concave points of each type of diagonsis (B =benign and M= malignant). 
By looking at the values of min value, max value and mean value of concave points, we can see that the benign cancer samples have less concave points than the malignant ones.
...
The approach is similar to what we have done with radius, smoothness, compactness or concave points. My goal for this data analysis is to understand the difference between benign and maligant cancer samples based on the provided variables. 

###### 1. Summarizing
Create a categorical variable with 3 or more groups from an existing numerical variable. You can use this new variable in the other tasks! *An example: age in years into "child, teen, adult, senior".*

Since my original dataset only contain one catogorical variable (diagnosis), therefore, I want to create a new categorical variable as symmery levels: low,  moderate, high and very high To determine which numeric threshold of each level. I calculated the minimum, first quartile (Q1), median (Q2), mean, third quartile (Q3), and maximum values for symmetry variable.
...
These data obtain the part 1.2 can directly give us an overview when comparing number of concave points of each type of diagonsis (B =benign and M= malignant). 
By looking at the values of min value, max value and mean value of concave points, we can see that the benign cancer samples have less concave points than the malignant ones. 
Since the malignant sample have larger radius than benign samples (results in question 1), we can rephrase the results that that larger radius samples (malignant) have more concave points than smaller radius sample (benign). 
...
Besides, the texture and fractional dimension variables are still remained unclear to us because we do not know which parameters are used to measure or calculate the values. I took a look on UCI Machine Learning Repository where the database was downloaded, however, there is no information about this. Therefore, it is difficult to understand to compare these two variables between two types of diagnosis. 
<!----------------------------------------------------------------------------->
...
We can see that the number of rows is same as number of samples in database
...
Each cell in the database contain only one value corresponding to the one sample and its variable. 
...
As we can see now in the table above, when we untidy the data, the number of rows increases and the multiple rows describe one sample instead of one row for each observation. 

#### No we tidy the data again: 
...
By using pivot_wider(), I was able to edit the data to be tidy again (original format). 
<!----------------------------------------------------------------------------->
...
- For the second question, I can also see that the difference in the number of concave points between two types of diagnosis. It is predicted that there is a proprotional relationship between radius_mean and concave_point_mean. It is a good option to do more modelling (regression model) or other statistical test on this prediction. 
<!----------------------------------------------------------------------------->
...
cancer_concave <- cancer_sample %>% 
  select(ends_with("mean")) %>% #only select the variables which end with 'mean'
  filter(!is.na(concave_points_mean)) %>%  # filter NA values in radius_mean column
...
**Research Question**: Are the larger radius sample likely to have more concave points than the smaller radius cancer  sample?
...
Now we will visualize the relationship between concave_points_mean and radius
